...
CreateThread(function()
    ...
    for job, data in pairs(wx.Jobs) do
        local whitelist 
        if data.whitelisted then whitelist = 1 else whitelist = 0 end


        local result = MySQL.Sync.fetchAll("SELECT label, whitelisted FROM jobs WHERE name = @name", {
            ['@name'] = job
        })
        ...
        if result[1] then
            ...
            if json.encode(result[1].label):gsub('"','') ~= data.label then
                MySQL.update('UPDATE jobs SET label = @label WHERE name = @name', {
                    ['@label'] = data.label,
                    ['@name'] = job
            ...
            if tostring(data.whitelisted) ~= json.encode(result[1].whitelisted) then
                MySQL.update('UPDATE jobs SET whitelisted = @whitelisted WHERE name = @name', {
    ...
    for job, jobData in pairs(wx.Jobs) do
        for number, gradeData in pairs(jobData.grades) do
            local result = MySQL.Sync.fetchAll("SELECT label, salary, grade, name FROM job_grades WHERE job_name = @job_name AND grade = @grade", {
                ['@job_name'] = job,
                ['@grade'] = number
            })
            ...
            if result[1] then
                ...
                if json.encode(result[1].label):gsub('"','') ~= gradeData.label then
                    MySQL.update('UPDATE job_grades SET label = @label WHERE job_name = @job_name AND grade = @grade', {
                        ['@label'] = gradeData.label,
                        ['@job_name'] = job,
                        ['@grade'] = number
                    }, function(num)
                        if num > 0 then
                            debug(("Label of %s of %s has been updated to %s"):format(number, job, gradeData.label), "info")
                        end
                    end)
                end
                ...
                if tonumber(json.encode(result[1].salary)) ~= gradeData.salary then
                    MySQL.update('UPDATE job_grades SET salary = @salary WHERE job_name = @job_name AND grade = @grade', {
                        ...
                        if num > 0 then
                            debug(("Salary of %s of %s has been updated to %s"):format(gradeData.label, job, gradeData.salary), "info")
                        end
                    end)
                end

                if json.encode(result[1].name):gsub('"','') ~= gradeData.id then
                    MySQL.update('UPDATE job_grades SET name = @name WHERE job_name = @job_name AND grade = @grade', {
                        ['@name'] = gradeData.id,
                        ['@job_name'] = job,
                        ['@grade'] = number
                    }, function(num)
                        if num > 0 then
                            debug(("Grade %s of %s has been updated to %s"):format(number, job, gradeData.id), "info")
                        end
                    end)
                end
            else
                newGrades = newGrades + 1
                MySQL.insert('INSERT INTO `job_grades` (job_name, name, grade, label, salary, skin_male, skin_female) VALUES (?, ?, ?, ?, ?, ? ,?)', {
                    job, gradeData.id, number, gradeData.label, gradeData.salary, "{}", "{}"
                })
            end
        end
    end
